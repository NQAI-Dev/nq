...
static void test_key_down_up_pump(void) {
    ...
    NQ_ASSERT_EQ(nq_input_key_down(&in, down.key.scancode), 1);
    nq_input_begin_frame(&in);
    NQ_ASSERT_EQ(nq_input_pump_sdl3_event(&in, &up),   1);
    NQ_ASSERT_EQ(nq_input_key_down(&in, up.key.scancode), 0);
    NQ_ASSERT_EQ(nq_input_key_released(&in, up.key.scancode), 1);
}
